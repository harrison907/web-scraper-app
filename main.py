...
<head>
    ...
    <style>
        ...
        .rank-badge { 
            ...
            justify-content: center; font-size: 15px; font-weight: 800; color: white; 
...
<body>
    ...
    <script>
        ...
        function renderList() {
            ...
            if(list.length === 0) {
                content.innerHTML = '<div style="text-align:center; padding:50px; color:#999;">暂无数据</div>';
                return;
            }
            ...
            content.innerHTML = list.map((m, index) => `
                <a href="${m.link}" class="movie-card" target="_blank">
                    ...
                    <div class="info-content">
                        ...
                        <div class="row-meta">
                            <span class="score">${m.score > 0 ? '★ ' + m.score : '新片待定'}</span>
...
def scrape():
    ...
    try:
        ...
        movie_data = []
        for m in raw_list:
            # 改进华语识别：猫眼字段中有 'fra' (来源国)，或者通过版本识别
            region = m.get('fra', m.get('src', ''))
            is_cn = any(x in region for x in ["中国", "大陆", "香港", "台湾"])
            
            movie_data.append({
                "title": m.get('nm', '未知'),
                "score": float(m.get('sc', 0)),
                "release_date": m.get('rt', '2026-01-01'), # 默认日期防止报错
